...
void CDraw::Avatar(int x, int y, int w, int h, uint32_t nFriendID)
{
	if (!I::SteamFriends || !I::SteamUtils)
		return;

	// Convert friendsID to Steam64 ID
	const uint64_t nID = static_cast<uint64_t>(nFriendID) + 0x0110000100000000ULL;

	// Check if avatar is already cached
	auto it = m_mapAvatars.find(nID);
	if (it != m_mapAvatars.end())
	{
		// Avatar already cached, just draw it
		I::MatSystemSurface->DrawSetColor(255, 255, 255, 255);
	...
	// Fetch avatar from Steam and cache it
	CSteamID steamID(nID);
	...
	if (!I::SteamUtils->GetImageSize(nAvatar, &nWidth, &nHeight))
		return;

	if (nWidth == 0 || nHeight == 0)
		return;

	const int nSize = static_cast<int>(4 * nWidth * nHeight * sizeof(uint8));
	uint8* pData = static_cast<uint8*>(malloc(nSize));
	...
	if (I::SteamUtils->GetImageRGBA(nAvatar, pData, nSize))
	{
		...
		{
			I::MatSystemSurface->DrawSetTextureRGBAEx(nTextureID, pData, nWidth, nHeight, IMAGE_FORMAT_RGBA8888);
...
void CDraw::RenderLine(const Vec3& vStart, const Vec3& vEnd, Color_t tColor, bool bZBuffer)
{
	...
	// Use RenderUtils for proper 3D line rendering (like Amalgam)
	RenderUtils::RenderLine(vStart, vEnd, tColor, bZBuffer);
}
	...
	for (size_t i = 1; i < vPath.size(); i++)
	{
		...
		case G::PathStyle::Separators:
		{
			...
			if (!(i % iSeparatorSpacing))
			{
				...
				if (flLen > 0.01f)
				{
					...
					Vec3 vPerp = { -vDir.y * flSeparatorLength, vDir.x * flSeparatorLength, 0.f };
					RenderLine(vEnd, vEnd + vPerp, tColor, bZBuffer);
				}
			}
			break;
		}
		case G::PathStyle::Spaced:
		{
			if (!(i % 2))
				RenderLine(vPath[i - 1], vPath[i], tColor, bZBuffer);
			break;
		...
		case G::PathStyle::Arrows:
		{
			if (!(i % 3))
			{
				...
				{
					Vec3 vAngles = Math::CalcAngle(vStart, vEnd);
					Vec3 vForward, vRight;
					Math::AngleVectors(vAngles, &vForward, &vRight, nullptr);
					RenderLine(vEnd, vEnd - vForward * 5.f + vRight * 5.f, tColor, bZBuffer);
					RenderLine(vEnd, vEnd - vForward * 5.f - vRight * 5.f, tColor, bZBuffer);
				}
			}
			break;
		...
		case G::PathStyle::Boxes:
		{
			...
			if (!(i % iSeparatorSpacing))
			{
				// Draw a small box at this point
				I::DebugOverlay->AddBoxOverlay(vPath[i], { -1, -1, -1 }, { 1, 1, 1 }, { 0, 0, 0 }, 
					tColor.r, tColor.g, tColor.b, tColor.a, 0.0f);
			}
			break;
		}
		}
...
void CDraw::RenderBox(const Vec3& vOrigin, const Vec3& vMins, const Vec3& vMaxs, const Vec3& vAngles, Color_t tColor, bool bZBuffer)
{
	if (!tColor.a)
		return;

	QAngle angles = { vAngles.x, vAngles.y, vAngles.z };
	RenderUtils::RenderWireframeBox(vOrigin, angles, vMins, vMaxs, tColor, bZBuffer);
}
...
void CDraw::DrawStoredPaths()
{
	...
	for (const auto& tPath : G::PathStorage)
	{
		...
		RenderPath(tPath.m_vPath, tPath.m_tColor, tPath.m_bZBuffer, tPath.m_iStyle, tPath.m_flTime);
	}

	// Draw all stored lines
	for (const auto& tLine : G::LineStorage)
	{
		if (tLine.m_flTime < I::GlobalVars->curtime)
			continue;

		RenderLine(tLine.m_vStart, tLine.m_vEnd, tLine.m_tColor, tLine.m_bZBuffer);
	}

	// Draw all stored boxes
	for (const auto& tBox : G::BoxStorage)
	{
		if (tBox.m_flTime < I::GlobalVars->curtime)
			continue;

		RenderBox(tBox.m_vOrigin, tBox.m_vMins, tBox.m_vMaxs, tBox.m_vAngles, tBox.m_tColorEdge, tBox.m_bZBuffer);
	}
}
